...
async function imgFromDB(key) {
  if (!key) return "";
  if (key.startsWith("data:")) return key; // 旧格式base64
...
const DEFAULT_DATA = {
  siteName: "PORTFOLIO",
  hero: { greeting: "Hello, 我是", name: "张三", tagline: "视觉设计师 / 插画师", bgGradient: "linear-gradient(135deg, #0f172a, #334155)" },
  about: { avatar: "", name: "张三", tagline: "视觉设计师 / 插画师", bio: "你好！我是一名热爱视觉设计的创作者。", info: {
    年龄: "28", 所在地: "上海", 电话: "138-0000-0000", 邮箱: "hello@example.com",
    学历: "设计硕士 / 某美术学院", 专长: "品牌设计 / UI设计 / 插画",
    工作经历: "2022-至今 某设计公司 高级设计师\n2020-2022 某互联网公司 UI设计师",
    科研经历: "2023 参与《设计趋势研究》课题\n2021 发表《UI设计中的色彩心理学》",
    获奖: "2024 年度最佳设计师奖\n2023 国际设计双年展入选",
  } },
  categories: ["平面设计", "品牌设计"],
  works: [
    { title: "作品示例 1", category: "平面设计", type: "image", images: [], videos: [] },
    { title: "作品示例 2", category: "品牌设计", type: "image", images: [], videos: [] },
  ],
};
...
async function storeFile(file) {
  const key = rndKey("img");
  await dbSet("images", key, file);
  return key;
}
...
function renderWorkCards(category) {
  ...
  grid.innerHTML = filtered.map(w => {
    ...
    const imgKey = w.images?.[0] || "";
    const thumb = isVideo ? (vidThumb || 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300"><rect fill="%23111" width="400" height="300"/><text x="200" y="140" text-anchor="middle" font-family="sans-serif" font-size="48" fill="%23a78bfa">▶</text><text x="200" y="180" text-anchor="middle" font-family="sans-serif" font-size="12" fill="%23666">视频</text></svg>')) : phImg;
    return `<div class="work-card" data-index="${idx}" data-reveal data-img="${escAttr(isVideo?'':imgKey)}">
      ...
      <div class="work-card-img-wrap"><img class="work-card-img" src="${thumb}" alt="${escHtml(w.title)}"></div>
  ...
  // 异步加载卡片图片
  grid.querySelectorAll("[data-img]").forEach(card => { const key = card.dataset.img; if (key) imgFromDB(key).then(url => { if (url) { const img = card.querySelector(".work-card-img"); if (img) img.src = url; } }); });
  initScrollReveal();
}
...
async function openDetail(index) {
  ...
  if (isVideo && w.videos?.length) {
    ...
    for (const v of w.videos) {
      ...
      if (blob) vids.push(`<video controls playsinline style="max-width:75vw;border-radius:var(--radius);box-shadow:0 8px 48px rgba(0,0,0,.5)"><source src="${URL.createObjectURL(blob)}" type="video/mp4"></video>`);
    }
    vidHtml = vids.join("");
  }
...
function initEditUI() {
  ...
  function enter() {
    ...
    ["heroGreet", "heroName", "heroTagline", "navLogo", "aboutName", "aboutTagline", "aboutBio"].forEach(id => { const el = document.getElementById(id); if (el) { el.contentEditable = "true"; el.title = "直接修改，失焦保存"; el.onblur = saveInline; } });
...
function saveInline() {
  ...
  if (g) DATA.hero.greeting = g.textContent; if (n) DATA.hero.name = n.textContent; if (t) DATA.hero.tagline = t.textContent; if (l) DATA.siteName = l.textContent;
  const an = document.getElementById("aboutName"), at = document.getElementById("aboutTagline"), ab = document.getElementById("aboutBio");
  if (an) DATA.about.name = an.textContent; if (at) DATA.about.tagline = at.textContent; if (ab) DATA.about.bio = ab.textContent;
  saveData();
}
...
function editAbout() {
  ...
  const infoHtml = fields.map(k => `<div class="form-group" style="margin-bottom:8px"><label>${k}</label><input id="edInfo_${k}" value="${escAttr(info[k] || "")}" style="width:100%;padding:10px 14px;border:1px solid var(--border);border-radius:6px;font-family:var(--font);font-size:.9rem;background:var(--bg2);color:var(--text)}"></div>`).join("");
